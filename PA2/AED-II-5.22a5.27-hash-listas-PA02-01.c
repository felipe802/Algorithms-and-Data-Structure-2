...
int main(int argc, char *argv[]) {
  ...
  LerPalavra(Elemento.Chave,N);
  while (strcmp(Elemento.Chave, "aaaaaa") != 0) {
    i = (Pesquisa(Elemento.Chave, p, Tabela) == NULL);
    if (i) printf("pesquisa sem sucesso \n");
    else printf("sucesso \n");
    printf("Pesquisar :  ");
    LerPalavra(Elemento.Chave,N);
  }
...
/*
a hash table with separate chaining, "not" collision handling
space complexity O(m + n), m = table size, n = # of elements
if the "fator de carga alpha" is small, O(m)

time complexity O(1). worst case all keys in the same list O(n)
*/